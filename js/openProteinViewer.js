	...
	if(typeof this.glmol0.atoms !== "undefined" && typeof this.glmol1.atoms !== "undefined"){
		if( molecule.id == "glmol0" ){
			...
			//both are normal aminoacid sequence loaded from PDB files
				...
				$('#glmol1_seq').html(string1);
				$('#glmol1_seq').jqDock('destroy');
				$('#glmol1_seq').jqDock({size:10, sizeMax:65, distance:20});
			}
		}
		if( molecule.id == "glmol1"){
			...
			//both are normal aminoacid sequence loaded from PDB files
				...
				$('#glmol0_seq').html(string0);
				$('#glmol0_seq').jqDock('destroy');
				$('#glmol0_seq').jqDock({size:10, sizeMax:65, distance:20});
			}
		}
	}
...
TuftsViewer.prototype.highLightAlignedMolecule = function(mol,atomlist){
	...
	if(mol.id === "glmol0"){
		this.saveAtomsColor0 = this.glmol0.savaAtomsColor(atomlist);
	}
	if(mol.id === "glmol1"){
		this.saveAtomsColor1 = this.glmol1.savaAtomsColor(atomlist);
	}
...
TuftsViewer.prototype.proteinAlignment = function(){
	...
	$.ajax({
		...
		success:function(response){
			...
			obj.parseAlignmentText(response);
...
TuftsViewer.prototype.loadAlignedMolecules = function() {
	...
	//math Glmol instance with the correct sequence
	...
	$('#glmol0_seq').html(str0);
	$('#glmol0_seq').jqDock('destroy');
	$('#glmol0_seq').jqDock(dockOptions);
	
	$('#glmol1_seq').html(str1);
	$('#glmol1_seq').jqDock('destroy');
	$('#glmol1_seq').jqDock(dockOptions);
};
...
TuftsViewer.prototype.enterImage = function(mol,imgIndex){
	if(mol.id === "glmol0"){
		$("#glmol1_seq img").eq(imgIndex).jqDock("expand");
	}
	if(mol.id === "glmol1"){
		$("#glmol0_seq img").eq(imgIndex).jqDock("expand");
	}
};
//remote control of another sequence, onmouseout event 
TuftsViewer.prototype.leaveImage = function(mol){
	if(mol.id === "glmol0"){
		$('#glmol1_seq.jqDocked').jqDock('idle');
		$("#glmol1_seq").trigger("docknudge");
		$("#glmol1_seq").show();
	}
	if(mol.id === "glmol1"){
		$('#glmol0_seq.jqDocked').jqDock('idle');
		$("#glmol0_seq").trigger("docknudge");		
		$("#glmol0_seq").show();
	}
};
